...
def run_nn(path_dict: dict,
    ...
    initial, com_thr, diff_ele, sym_ele, sym_flap = \
    ...
    results = np.zeros((len(diff_ele) + input_timesteps, input_shape))
    ...
    for i_count in tqdm(range(0, len(diff_ele))):
        ...
        if nn_mode == "feedforward":
            ...
            temp[:, input_shape * (input_timesteps - 1) + output_shape] = com_thr[i_count]
            temp[:, input_shape * (input_timesteps - 1) + output_shape + 1] = diff_ele[i_count]
            temp[:, input_shape * (input_timesteps - 1) + output_shape + 2] = sym_ele[i_count]
            temp[:, input_shape * (input_timesteps - 1) + output_shape + 3] = sym_flap[i_count]
...
#for_spe,AofA,pit_rate,pitch,throt,com_thr,diff_ele,sym_ele,sym_flap
            ...
            temp[0, input_timesteps - 1, output_shape] = com_thr[i_count]
            temp[0, input_timesteps - 1, output_shape + 1] = diff_ele[i_count]
            temp[0, input_timesteps - 1, output_shape + 2] = sym_ele[i_count]
            temp[0, input_timesteps - 1, output_shape + 3] = sym_flap[i_count]

        new_input = temp

    results[:, output_shape:input_shape] = data[startpoint:startpoint + len(com_thr) + input_timesteps,
